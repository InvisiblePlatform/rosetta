...
def process_document(document):
    ...
    for alias_entry in aliases:
        ...
        for alias_varient in [alias, stripped_alias, companyless_alias, corpless_alias]:
            if alias_varient in data:
                matched_label_id = data[alias_varient].get('id', '')
    ...
    if len(website) > 0:
        result[oid] = {"website": website, "id": oid, "osid": openSecrets, "label": label.get('value', '') if isinstance(label, dict) else label}
    ...
    with Pool(initializer=init_progress_bar, initargs=(pbar,)) as pool:
        ...
        for result in results:
            if result:
                ...
                if isinstance(result[oid]["osid"], list):
