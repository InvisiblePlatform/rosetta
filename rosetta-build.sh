...
# wikidata/graph-parts/graph-Q355.json wikidata/graph-parts/graph-Q350.json > $TEMP

# jq -s 'map(to_entries)|flatten|group_by(.key)|map({(.[0].key):map(.value)|add})|add'  \
# wikidata/graph-parts/graph-Q1336.json $TEMP

# sitehash="fd33606649281874a091e4c1d0243953"
FIELDS=( "bcorp" "emailaddress" "facebookid" "facebookpage" "gabuser" 
    "glassdoor" "goodonyou" "instagramid" "isin" "mastodon" "mbfc" "medium" 
    "officialblog" "parleruser" "patreon" "polalignment" "polideology" 
...
update_with_hash(){
    ...
    local TEMP; local TEMP2; local TEMP3
    TEMP=$(mktemp)
    TEMP2=$(mktemp)
    TEMP3=$(mktemp)
   ...
   if grep -q "^$field:" "$TEMP"; then
       local tempval
       tempval=$(grep "^$field:" "$TEMP" | head -1)
       sed -i "/^$field:/d" "$TEMP"
       printf '%s\n' "$tempval" >> "$TEMP"
   fi
   done
